...
export const createPrixer = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {

    if (!req.permissions?.prixerBan) {
      res.status(403).send({ success: false, message: "No tienes permiso para crear perfiles de Prixer." });
      return;
    }

    const userId = req.params.userId;
    if (!userId) {
      res.status(400).send({ success: false, message: "User ID is required to create a Prixer profile." });
      return;
    }
    ...
    try {
      ...
      res.status(400).send({ success: false, message: "Invalid User ID format." });
      return;
    }
    ...
    const specialty = req.body.specialty
      ? (Array.isArray(req.body.specialty)
        ? req.body.specialty
        : (req.body.specialty as string).split(',').map(s => s.trim()).filter(s => s.length > 0))
      : undefined;
    ...
    const result = await prixerServices.createPrixer({ userId: userId, ...prixerData });
...
export const getPrixerByUsername = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const readAllPrixers = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    if (!req.permissions?.prixerBan) {
      res.send({ success: false, message: "No tienes permiso para editar un Prixer." });
      return;
    }
...
export const readAllPrixersActive = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const updatePrixer = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    if (!req.permissions?.prixerBan) {
      res.send({ success: false, message: "No tienes permiso para editar un Prixer." });
      return;
    ...
    const avatarUploads = (req.session?.uploadResults as any)?.avatar as
      | { purpose: string; url: string }[]
      | undefined;
    const avatarUrl = avatarUploads?.find((u) => u.purpose === "PrixerAvatar")?.url;
...
export const updateVisibility = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    if (!req.permissions?.prixerBan) {
      res.send({ success: false, message: "No tienes permiso para cambiar visibilidad." });
      return;
    }
...
export const updateTermsAgreeGeneral = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    if (!req.permissions?.prixerBan) {
      res.send({ success: false, message: "No tienes permiso para perfiles globales." });
      return;
    }
    const termsAgree = req.body.termsAgree === "true" || req.body.termsAgree === true;
...
export const updateTermsAgree = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const termsAgree = req.body.termsAgree === "true" || req.body.termsAgree === true;
    const result = await prixerServices.updateTermsAgree(req.params.id, termsAgree);
    res.send(result);
  } catch (e) {
    console.error(e);
    next(e);
...
export const destroyPrixer = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    if (!req.permissions?.prixerBan) {
      res.send({ success: false, message: "No tienes permiso para eliminar un Prixer." });
      return;
    }
