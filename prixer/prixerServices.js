...
const createPrixer = async (prixerData)=> {
    ...
    try {
        ...
        return {
            success: false,
            message: e + 'Disculpa. No se pudo cargar tus datos de Prixer, inténtalo de nuevo por favor.'
        }
    }
}
...
const readAllPrixersFull = async ()=> {
    ...
    let data = [];
    if(readedPrixers){
        data = await Promise.all(readedPrixers.map( async (readedPrixer)=>{
            const readedUser = await userService.readUserById({'id': readedPrixer.userId});
            if(readedUser) {
                const prixer = mergePrixerAndUser(readedPrixer, readedUser);
                return prixer;
            } else {
                return {
                    info: 'El Prixer ' + readedPrixer.username + ' no está asignado a un usuario.',
                    prixers: null
                }
            }
        }));
...
const updatePrixer = async (prixerData, userData)=> {
    ...
    if(!updatedPrixer) { return console.log('Prixer update error: '+err); }
...
module.exports = {createPrixer, readAllPrixers, readPrixer, updatePrixer, disablePrixer, removePrixers, readAllPrixersFull};