import { Product, Variant } from "./productModel";
import * as orderServices from "../order/orderService";
import { readAllDiscounts } from "../discount/discountServices";
import * as ArtService from "../art/artServices";
...
import { applyAdjustmentsToList, applyAdjustmentsToProduct, getAdjustmentValue, isDateActive } from "./utils";
...
export const readById = async (id: string): Promise<PrixResponse> => {
  try {
    ...
    return { success: true, message: "Producto encontrado.", result: updatedProd };
  } catch (e: unknown) {
    return { success: false, message: `Error: ${(e as Error).message}` };
  }
};
...
export const readActiveById = async (id: string): Promise<PrixResponse> => {
  try {
    ...
    const product = await products.findOne({ _id: productObjectId, active: true });

    // Check if product was found and is active
    if (!product) {
      return { success: false, message: "Producto no encontrado o no está activo." };
    }
    ...
    if (!discountsRes.success || !surchargesRes.success || !Array.isArray(surchargesRes.result)) {
      console.warn(`readActiveById: Could not fetch discounts/surcharges for product ${id}. Returning product without adjustments or price calculations.`);
      return { success: true, message: "Producto activo encontrado (ajustes/recargos no pudieron ser procesados).", result: product };
    }
    ...
    if (productWithAdjustmentIds.variants && productWithAdjustmentIds.variants.length > 0) {
      productWithAdjustmentIds.variants = productWithAdjustmentIds.variants.map(variant => {
        if (variant.surchargeId && variant.surchargeId.length > 0) {
          ...
          if (isNaN(basePublicPrice) || isNaN(basePrixerPrice)) {
            console.warn(`readActiveById: Invalid base price for variant ${variant._id} in product ${product._id}. Skipping surcharge calculation for this variant.`);
            return variant;
          }
          ...
          variant.surchargeId.forEach(surchargeId => {
            ...
            if (surcharge) {
              ...
              console.warn(`readActiveById: Surcharge details not found for ID ${surchargeId} during price calculation.`);
            }
          });
    ...
    return { success: true, message: "Producto activo encontrado.", result: productWithAdjustmentIds };

  } catch (e: unknown) {
    console.error(`Error in readActiveById for id ${id}:`, e);
    const message = e instanceof Error ? e.message : String(e);
    return { success: false, message: `Error retrieving active product by ID: ${message}` };
  }
};

export const getVariantPrice = async (variantId: string, productId: string, user?: User | null, isPrixer?: boolean): Promise<PrixResponse> => {
  try {
    const products = productCollection();
    let productObjectId: ObjectId;
    try {
      ...
      console.error(`Invalid productId format for getVariantPrice: ${productId}`, error);
      return { success: false, message: "Formato de ID de producto inválido." };
    }
    ...
    if (!product?.variants?.length) {
      console.warn(`getVariantPrice: Product or variants not found for productId: ${productId}`);
    ...
    if (!variant) {
      console.warn(`getVariantPrice: Variant not found for variantId: ${variantId} within productId: ${productId}`);
      return { success: false, message: "Variante no encontrada." };
    }

    const priceSourceKey = isPrixer ? 'prixerPrice' : 'publicPrice';
    const priceStringToParse = variant[priceSourceKey as keyof Pick<Variant, 'publicPrice' | 'prixerPrice'>] as string;

    const basePrice = parseFloat(priceStringToParse);

    if (isNaN(basePrice)) {
      return { success: false, message: "Precio base inválido para esta variante." };
    }
    ...
    (activeSurcharges.result as Surcharge[]).forEach(surcharge => {
      ...
      const isApplicable = surcharge.appliesToAllProducts ||
        surcharge.applicableProducts?.some(([pId, vId]) =>
          pId === productIdString && (!vId || vId === variantId));
    ...
    (activeDiscounts.result as Discount[]).forEach(discount => {
      ...
      const isApplicable = discount.appliesToAllProducts ||
        discount.applicableProducts?.some(([pId, vId]) =>
          pId === productIdString && (!vId || vId === variantId));
    ...
    // Round final values
    const finalOriginalPrice = parseFloat(originalPriceWithSurcharges.toFixed(2));
    ...
    console.error(`getVariantPrice: Error calculating price for productId: ${productId}, variantId: ${variantId}`, e);
    const errorMessage = e instanceof Error ? e.message : String(e);
    return { success: false, message: `Error al calcular precios de variante: ${errorMessage}` };
  }
};
...
export const readAllProducts = async (): Promise<PrixResponse> => {
  try {
    ...
    return { success: true, message: "Productos encontrados.", result: adjustedList };
  } catch (e: unknown) {
    return { success: false, message: `Error: ${(e as Error).message}` };
  }
};
...
export const readAllActiveProducts = async (): Promise<PrixResponse> => {
  try {
    ...
    if (!surchargesRes.success || !Array.isArray(surchargesRes.result)) {
      console.warn("readAllActiveProducts: Could not fetch surcharge details for price calculation.");
      return { success: true, message: "Productos activos encontrados (surcharge price calculation skipped due to fetch error).", result: listWithAdjustmentIds };
    }
      ...
      if (product.variants && product.variants.length > 0) {
        const updatedVariants = product.variants.map(variant => {
          if (variant.surchargeId && variant.surchargeId.length > 0) {
            ...
            if (isNaN(basePublicPrice) || isNaN(basePrixerPrice)) {
              console.warn(`readAllActiveProducts: Invalid base price for variant ${variant._id} in product ${product._id}. Skipping surcharge calculation for this variant.`);
              return variant;
            }
            ...
            variant.surchargeId.forEach(surchargeId => {
              ...
              if (surcharge) {
                ...
                console.warn(`readAllActiveProducts: Surcharge details not found for ID ${surchargeId} during price calculation.`);
              }
            });
    ...
    return { success: true, message: "Productos activos encontrados.", result: listWithUpdatedPrices };

  } catch (e: unknown) {
    console.error("Error in readAllActiveProducts:", e);
    const message = e instanceof Error ? e.message : String(e);
    return { success: false, message: `Error retrieving active products: ${message}` };
  }
};
...
export const readBestSellers = async (): Promise<PrixResponse> => {
  try {
    ...
    if (!ordersResp.success || !Array.isArray(ordersResp.result)) {
      return { success: false, message: "Error fetching orders for best sellers." };
    }
    const orders = ordersResp.result as Order[];

    const allPotentialProducts = await products
      .find({ active: true, variants: { $exists: true, $not: { $size: 0 } } }).toArray();

    const productMapByName = new Map<string, Product>(allPotentialProducts.map(p => [p.name, p as Product]));
    ...
    return { success: false, message: `Error calculating best sellers: ${(e instanceof Error ? e.message : String(e))}` };
  }
};

export const updateProduct = async (id: string, update: Partial<Product>): Promise<PrixResponse> => {
  try {
    const products = productCollection();
...
export const deleteVariant = async (data: { product: string; variant: string }): Promise<PrixResponse> => {
  try {
    const products = productCollection();
    ...
    return { success: true, message: "Variante eliminada exitosamente.", result: result! };
  } catch (e: unknown) {
    return { success: false, message: `Error: ${(e as Error).message}` };
  }
};