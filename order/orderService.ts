...
export const readOrdersByEmail = async (data: {
  ...
  try {
    const order = orderCollection()
    const orders = await order
      .find({ "consumerDetails.basic.email": data.email })
      .toArray()
    return orders.length
...
export const updateOrder = async (
  ...
  try {
    ...
    if (shouldProcessCommissions) {
      ...
      console.log(
        `Orden ${updatedOrder._id || updatedOrder.number} marcada como 'commissionsProcessed'.`
      )
    } else if (updatedOrder.commissionsProcessed) {
      console.log(
        `Orden ${updatedOrder._id || updatedOrder.number} ya tiene comisiones procesadas. Saltando.`
      )
    }
...
export const createPaymentMethod = async (
  ...
  try {
    ...
    const { acknowledged, insertedId } =
      await paymentMethods.insertOne(methodToInsert)
...
export const readPaymentMethodById = async (
  id: string
): Promise<PrixResponse> => {
  try {
...
export const createShippingMethod = async (
  ...
  try {
    ...
    const methodToInsert = {
      ...shippingMethodData,
      active:
        shippingMethodData.active !== undefined
          ? shippingMethodData.active
          : true,
      createdOn: new Date(),
    }

    const { acknowledged, insertedId } =
      await shippingMethods.insertOne(methodToInsert)
...
export const readShippingMethodById = async (
  id: string
): Promise<PrixResponse> => {
  try {
