...
export const createOrder4Client = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const addVoucher = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!voucherUrl) {
      res.status(400).send({ success: false, message: "No voucher found in session." });
      return;
    }
...
export const readOrdersByEmail = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
...
export const updateOrder = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    if (!orderId) {
      res.status(400).send({ success: false, message: "ID de orden requerido." });
      return;
    }
    if (Object.keys(updateData).length === 0) {
      res.status(400).send({ success: false, message: "No se proporcionaron datos para actualizar." });
      return;
    }
...
export const getGlobalOrders = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (startDate > endDate) {
      res.status(400).send({ success: false, message: "Start date cannot be after end date." });
      return;
    }

    const result = await orderServices.readAllOrdersByDateRange(startDate, endDate);
    res.send(result);
  } catch (err) {
...
export const getGlobalDashboardStats = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 29);
    thirtyDaysAgo.setHours(0, 0, 0, 0);
    ...
    // @ts-ignore
    const result = await orderServices.calculateGlobalDashboardStats(startDate, endDate);
    res.send(result);
  } catch (err) {
...
export const getGlobalTopPerformingItems = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 29);
    thirtyDaysAgo.setHours(0, 0, 0, 0);
    ...
    if (isNaN(limit) || limit <= 0) {
      res.status(400).send({ success: false, message: "Invalid limit provided. Must be a positive number." });
      return;
    }

    // @ts-ignore
    const result = await orderServices.getGlobalTopPerformingItems(startDate, endDate, limit);
    res.send(result);
  } catch (err) {
...
export const createPaymentMethod = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!methodData || !methodData.name) {
      res.status(400).send({ success: false, message: "Datos de método de pago incompletos (se requiere nombre)." });
      return;
    }
...
export const readPaymentMethod = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const resultPaymentMethod = await orderServices.readPaymentMethodById(req.params.id);
...
export const readAllActivePaymentMethods = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const active = true;
    const resultPaymentMethods = await orderServices.readAllActivePaymentMethods(active);
    res.send(resultPaymentMethods);
  } catch (err) {
    next(err);
  }
...
export const updatePaymentMethod = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!req.permissions?.createPaymentMethod) {
      res.status(403).send({ success: false, message: "No tienes autorización para actualizar métodos de pago." });
      return;
    }
    ...
    if (!methodId) {
      res.status(400).send({ success: false, message: "ID de método de pago requerido." });
      return;
    }
...
export const createShippingMethod = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!methodData || !methodData.name) {
      res.status(400).send({ success: false, message: "Datos de método de envío incompletos (se requiere nombre)." });
      return;
    }
...
export const readAllActiveShippingMethod = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const resultShippingMethods = await orderServices.readAllActiveShippingMethod();
    res.send(resultShippingMethods);
  } catch (err) {
    next(err);
  }
...
export const readShippingMethod = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const resultShippingMethod = await orderServices.readShippingMethodById(req.params.id);
...
export const updateShippingMethod = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {

    if (!req.permissions?.createShippingMethod) {
      res.status(403).send({ success: false, message: "No tienes autorización para actualizar métodos de envío." });
      return;
    }
    ...
    if (!methodId) {
      res.status(400).send({ success: false, message: "ID de método de envío requerido." });
      return;
    }
    if (!updateData || Object.keys(updateData).length === 0) {
      res.status(400).send({ success: false, message: "No se proporcionaron datos para actualizar." });
    ...
    const result = await orderServices.updateShippingMethod(methodId, updateData);
    res.send(result);
...
export const deleteShippingMethod = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    if (!req.permissions?.modifyAdmins) {
      res.status(403).send({ success: false, message: "No tienes autorización para eliminar métodos de envío." });
      return;
    }
