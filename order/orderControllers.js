...
const updateOrder = async (req, res)=> {
  try {
    const updatedOrder = await orderServices.updateOrder(req.body);
...
try {
  const readedPaymentMethod = await orderServices.readPaymentMethodByEmail(req.body);
...
try {
  const updatedPaymentMethod = await orderServices.updatePaymentMethod(req.body);
...
try {
  const readedOrderPayment = await orderServices.readOrderPaymentByEmail(req.body);
...
try {
  const updatedOrderPayment = await orderServices.updateOrderPayment(req.body);
  ...
  module.exports = { 
    createOrder, readOrder, readAllOrders, updateOrder,
    createPaymentMethod, readPaymentMethod, readAllPaymentMethods, updatePaymentMethod,
    createOrderPayment, readOrderPayment, readAllOrderPayments, updateOrderPayment,
     }