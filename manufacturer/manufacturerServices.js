...
const createManufacturer = async (manufacturerData) => {
  ...
  try {
    ...
    return {
      ...
      message:
        e +
        "Disculpa. No se pudo cargar tus datos de Manufacturer, inténtalo de nuevo por favor.",
    };
  }
};
...
const readManufacturer = async (manufacturerData) => {
  ...
  let readedManufacturer = await Manufacturer.findOne({
    userId: manufacturerData.id,
  }).exec();
  if (readedManufacturer) {
    const readedUser = await userService.readUserById({
      id: manufacturerData.id,
    });
    const manufacturer = await mergeManufacturerAndUser(
      readedManufacturer,
      readedUser
    );
    return manufacturer;
  ...
  try {
    ...
    if (readedManufacturers) {
      data = await Promise.all(
        readedManufacturers.map(async (readedManufacturer) => {
          const readedUser = await userService.readUserById({
            id: readedManufacturer.userId,
          });
          if (readedUser) {
            const manufacturer = mergeManufacturerAndUser(
              readedManufacturer,
              readedUser
            );
            return manufacturer;
