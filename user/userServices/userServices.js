...
const changePassword = async (userData)=> {
    try {
        const readedUserPassByUsername = await readUserPassByUsername(userData.username);
        ...
        const validatedCurrentPassword = await validateCurrentPassword(userData.currentPassword, readedUserPassByUsername.password);
...
const readUserByResetToken = async (data)=> { return await User.findOne({'token': data.token})};
const readUserByEmailNotExec = async(userData)=> { return await User.findOne({'email': userData.email})};
const readUserById = async(userData)=> { return await User.findOne({'_id': userData.id}).exec()};
const readUserByEmail = async(userData)=> { return await User.findOne({'email': userData.email}).exec()};
const readUserByUsername = async(username)=> { return await User.findOne({'username': username}).exec()};
const readUserPassByUsername = async(username)=> { return await User.findOne({'username': username}).exec()};
...
const updateUser = async (userData)=> {
    ...
    if(!updatedUser) { return console.log('User update error: '+err); }
    return updatedUser;
}
...
module.exports = {createUser, readUserById, readUserByEmailNotExec, readUserByEmail, simpleUserUpdate, readUserByUsername, updateUser, disableUser, removeUser, removeUsers, changePassword, resetPassword, readUserByResetToken};