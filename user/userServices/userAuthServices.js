...
const forgotPassword = async (email)=> {
    try {
        ...
        if(!user){
            return res.status(400).json({error: 'Este usuario no existe, inténtalo de nuevo o regístrate.'})
        }
...
const checkPasswordToken = async (token)=> {
    try {
        return jwt.verify(token, process.env.RESET_PASSWORD_KEY, (err, verifiedToken)=>{
...
module.exports = {authenticate, generateToken, forgotPassword, checkPasswordToken, resetPassword};