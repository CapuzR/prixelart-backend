    ...
    const uploadThumbnailArt = multer({
        storage: multerS3({
          ...
          transforms: [{
            id: 'squareThumb',
            key: function (req, file, cb) {
              cb(null, req.body.title + '-' + req.body.artId + '-square.webp')
            },
            ...
            id: 'largeThumb',
            key: function (req, file, cb) {
              cb(null, req.body.title + '-' + req.body.artId + '-large.webp')
            },
            transform: function (req, file, cb) {
            ...
            id: 'mediumThumb',
            key: function (req, file, cb) {
              cb(null, req.body.title + '-' + req.body.artId + '-medium.webp')
            },
            transform: function (req, file, cb) {
            ...
            id: 'smallThumb',
            key: function (req, file, cb) {
              cb(null, req.body.title + '-' + req.body.artId + '-small.webp')
            },
            transform: function (req, file, cb) {
      ...
      function originalArtCb(request, res, next) {
        return function (error, response) {
          if (error) {
            ...
            const publicArtUrl = process.env.PUBLIC_BUCKET_URL + '/' +
            request.body.title + '-' +
            request.body.artId;

            const privateArtUrl = process.env.PRIVATE_BUCKET_NAME + '.' +
