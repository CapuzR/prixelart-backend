...
export const readOrder = async (filter: Record<string, unknown>): Promise<PrixResponse> => {
    try {
        const orderArchives = orderArchiveCollection();
...
export interface PaginatedOrderArchiveResponse extends Omit<PrixResponse, 'result'> {
    result: {
        data: OrderArchive[];
        totalCount: number;
    } | null;
...
export const readAllOrders = async (options: OrderArchiveQueryOptions): Promise<PaginatedOrderArchiveResponse> => {
    try {
        const {
            page,
            limit,
        ...
        return {
            success: true,
            message: list.length ? "Órdenes recuperadas." : "No se encontraron órdenes para esta página/filtros.",
            result: {
                data: list,
...
export const addVoucher = async (id: string, url: string): Promise<PrixResponse> => {
    try {
        const orderArchives = orderArchiveCollection();
...
export const updateOrder = async (id: string, data: Partial<OrderArchive>, adminUsername: string): Promise<PrixResponse> => {
    try {
        const orderArchives = orderArchiveCollection();
        const result = await orderArchives.findOneAndUpdate(
            { _id: new ObjectId(id) },
        ...
        // If the status is being updated to "Concretado", handle commission payments
        if (data.status === "Concretado" && updatedOrder.status === "Concretado") {
            console.log(`Order ${updatedOrder.orderId} status is 'Concretado'. Processing commissions by ${adminUsername}.`);

            for (const request of updatedOrder.requests) {
                if (!request.art || !request.product) {
                    console.warn(`Skipping commission for a request in order ${updatedOrder.orderId} due to missing art or product data.`);
                    continue;
                }
                ...
                if (artSpecificCommission !== undefined && artSpecificCommission !== null) {
                    ...
                    if (!isNaN(parsedArtCommission) && parsedArtCommission > 0) {
                        ...
                        console.log(`Using specific commission rate ${parsedArtCommission}% for art '${request.art.title}' by ${request.art.prixerUsername}.`);
                    } else {
                        commissionRate = 0.10; // Default 10%
                        console.warn(`Invalid art commission ('${artSpecificCommission}') for art '${request.art.title}'. Defaulting to 10%.`);
                    }
                } else {
                    commissionRate = 0.10; // Default 10%
                    console.log(`No specific commission for art '${request.art.title}'. Using default 10% for ${request.art.prixerUsername}.`);
                }
                ...
                if (productFinalPriceStr !== undefined && requestQuantityStr !== undefined) {
                    const finalPrice = parseFloat(String(productFinalPriceStr));
                    const quantity = parseFloat(String(requestQuantityStr));
                    if (!isNaN(finalPrice) && !isNaN(quantity) && finalPrice > 0 && quantity > 0) {
                        requestSubtotal = finalPrice * quantity;
                    } else {
                        console.warn(`Cannot calculate subtotal for request (Art: '${request.art.title}'). Invalid finalPrice ('${productFinalPriceStr}') or quantity ('${requestQuantityStr}'). Skipping.`);
                        continue;
                    }
                } else {
                    console.warn(`Missing finalPrice or quantity for request (Art: '${request.art.title}'). Skipping.`);
                    continue;
                }
        ...
        return { success: true, message: "Order updated successfully.", result: updatedOrder };
    } catch (e) {
        console.error(`General error in updateOrder: ${makeError(e)}`);
        return { success: false, message: `An error occurred while updating the order: ${makeError(e)}` };
    }
};

export const updateItemStatus = async (id: string, itemIndex: number, status: string): Promise<PrixResponse> => {
    try {
        const orderArchives = orderArchiveCollection();
