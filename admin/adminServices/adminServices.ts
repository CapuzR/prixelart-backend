...
export const createAdmin = async (adminData: Admin): Promise<PrixResponse> => {
  try {
    ...
    const resultAdminByUsername = await admins.findOne({ username: adminData.username });
...
export const readSellers = async (): Promise<PrixResponse> => {
  try {
    ...
    const sellerNames = sellers.map(seller => `${seller.firstname} ${seller.lastname}`);
...
export const readAllAdmins = async (): Promise<PrixResponse> => {
  try {
    ...
    const admins = await adminsCollection.find<Admin>(
      {},
      { projection: { password: 0 } }
    ).toArray();
    const adminsWithoutPassword = admins.map(({ password, ...rest }) => rest) as Admin[];
...
export const readFullAdminByEmail = async (email: string): Promise<PrixResponse> => {
  try {
    const admins = adminCollection();
...
export const readAdminByEmail = async (email: string): Promise<PrixResponse> => {
  try {
    const admins = adminCollection();
...
export const getAdminByUsername = async (username: string): Promise<PrixResponse> => {
  try {
    const admins = adminCollection();
    const admin = await admins.findOne({ username: username }, { projection: { password: 0 } });
...
export const updateAdmin = async (username: string, adminData: Partial<Admin>): Promise<PrixResponse> => {
  try {
    const admins = adminCollection();

    const { username: newUsername, password, ...otherDataToUpdate } = adminData;

    if (newUsername != username) {
      return {
        success: false,
        message: "No se puede actualizar el nombre de usuario con esta función. Utilice una función dedicada si es necesario.",
      };
    }
...
export const deleteAdmin = async (adminUsername: string): Promise<PrixResponse> => {
  try {
    const admins = adminCollection();
...
export const readPermissionsById = async (id: string): Promise<PrixResponse> => {
  try {
    const permissiondb = permissionsCollection();
    let objectId;
...
export const updatePermissions = async (id: string, role: Partial<Permissions>): Promise<PrixResponse> => {
  try {
...
export const deletePermissions = async (id: string): Promise<PrixResponse> => {
  try {
    const permissiondb = permissionsCollection();
    const result = await permissiondb.findOneAndDelete({ _id: new ObjectId(id) });
    if (result) {
      return {
        success: true,
