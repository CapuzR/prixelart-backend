...
const createAdmin = async (adminData)=> {
    try {
        ...
        const readedAdminByUsername = await adminServices.readAdminByUsername(adminData.username);
...
const authenticate = async adminData => {
    ...
    if(admin) {
        ...
        return {
            ...
            error_message: 'No se encuentra el email, por favor solicita un usuario con acceso.'
        };
    }
// }
} catch(e) {
	...
	try{
		...
		if (!adminToken) {
			return res.send( {
					...
					error_message: 'No has iniciado sesión. Por favor inicia sesión para continuar.'
			});
		} else {
		jwt.verify(adminToken, process.env.JWT_SECRET, (err, decoded) => {
			if (err)
				return res.status(500).send({ auth: false, message: 'Falló autenticación de token.' });
		next();
		});
	}
	} catch(e) {
...
module.exports = {createAdmin, authenticate, generateToken, ensureAuthenticated};
