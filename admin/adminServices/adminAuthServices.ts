  ...
  try {
    ...
    if (admin.result) {
      ...
      if (!bcrypt.compareSync(adminData.password, adminResult.password!)) {
        ...
        const adminToken = jwt.sign(payload, secret, { expiresIn: tokenExpireTimeInSeconds });
      ...
      return {
        success: false,
        message: "No se encuentra el email, por favor solicita un usuario con acceso.",
      };
    }
  } catch (error) {
...
export const getPermissions = async (decoded: string): Promise<PrixResponse> => {
  try {
