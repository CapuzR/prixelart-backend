...
const readConsumerByEmail = async(consumerData)=> { return await Consumer.findOne({'email': consumerData.email}).select('-_id').exec()};
...
const readConsumerByUsername = async(username)=> { return await Consumer.findOne({'username': username}).exec()};
...
const updateConsumer = async (consumerData)=> {
    try{
        const toUpdateConsumer = await Consumer.findOne({'email': consumerData.email});
        await toUpdateConsumer.set(consumerData);
        const updatedConsumer = await toUpdateConsumer.save();
        if(!updatedConsumer) { return console.log('Consumer update error: '+err); }
...
module.exports = { createConsumer, readConsumerByEmail, readConsumerByQuery, readConsumerByUsername, readAllConsumers, updateConsumer }